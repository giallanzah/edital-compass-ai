...
-- Bloco 5: módulo de Consultor Fomenta.ai (CRM interno de consultores)

-- 1) Novo papel CONSULTOR no enum de roles já existente.
-- ALTER TYPE ... ADD VALUE não pode ser usado na mesma transação em que o
-- valor novo é referenciado — por isso nenhuma outra instrução deste arquivo
-- usa 'CONSULTOR' em DEFAULT/comparações fora de função (as funções abaixo
-- só executam depois que este ALTER já foi COMMITado).
ALTER TYPE public.app_role ADD VALUE IF NOT EXISTS 'CONSULTOR';

-- 2) Consultores credenciados (staff interno; nunca auto-cadastro).
...
-- Só leitura própria por RLS; toda escrita (credenciar/editar) passa pela
-- função credenciar_consultor() ou por supabaseAdmin no backoffice (ambos
-- bypassam RLS), então authenticated só precisa de SELECT.
...
-- 3) Vínculo consultor ↔ empresa cliente (contrato).
...
-- Único contrato ATIVO por par consultor/empresa — permite histórico de
-- contratos encerrados sem violar unicidade.
...
-- 4) Qual consultor está acompanhando cada candidatura.
...
-- Policies adicionais (permissivas, somadas às já existentes "*_own") para o
-- consultor ver/mover candidaturas e ver empresas/projetos da sua carteira,
-- e para a empresa ver o consultor responsável por ela.
...
-- Empresa vê o nome/contato do consultor responsável (contrato ativo) —
-- necessário para o portal mostrar "consultor responsável" na candidatura.
...
-- 5) Atividades do consultor (chamados de cliente + tarefas próprias, tipo CRM).
...
-- consultor_id nullable de propósito: um chamado de cliente pode nascer sem
-- consultor atribuído se a empresa ainda não tiver contrato ativo.
...
-- Empresa (cliente) cria seu próprio chamado — só INSERT, sem SELECT: o
-- retorno da chamada já basta para o botão "chamar consultor" confirmar.
...
-- 6) Credenciamento de consultor (admin) — vincula usuário já existente,
-- nunca cria conta nova. SECURITY DEFINER para poder gravar em user_roles e
-- consultores atomicamente; segue o mesmo padrão de promover_usuario().
